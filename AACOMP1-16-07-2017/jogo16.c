	...
	for(int i = 0;i < qtdTorres; i++)
    {
    	if(t1[i].estado == ATIVADA)
    	{
    		for (int j = 0; j < NUM_TIROS; j++)
    		{
    			...
    			{
    				t1[i].tiros[j].rect.x -= sin(90*M_PI/180.0)*5.0;
    				t1[i].tiros[j].rect.y -= cos(90*M_PI/180.0)*5.0; 
...
void tiroT1i(int i)
{
	if(i != -1)
	{
		for (int j = 0; j < NUM_TIROS; j++)
		{
	    	...
	    	}else if(t1[i].tiros[j].estado == DESATIVADO)
			{
				int x = t1[i].rect.x;
				int y = t1[i].rect.y;
