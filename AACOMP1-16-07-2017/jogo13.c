...
void atirar()
{
	for(int i = 0;i < qtdTorres; i++)
    {
    	if(t1[i].estado == ATIVADA)
    	{
    		...
    		for (int j = 0; j < NUM_TIROS; j++)
    		{
	    		if(t1[i].tiros[j].estado == ATIVADA)
	        	{
			...
			*/
    		for (int j = 0; j < NUM_TIROS; j++)
    		{
    			...
    			if(t1[i].tiros[j].estado == ATIRANDO)
    			{
    				...
    				//printf("angulo %f\n",sin(atan2(t1[i].rect.y-100,t1[i].rect.x-100)*M_PI/180.0)*1.0);

    				//int x,y;

    				//SDL_GetMouseState(&x,&y);

    				t1[i].tiros[j].rect.x += sin(atan2(200-100,200-100)*M_PI/180.0)*1.0;
	            	t1[i].tiros[j].rect.y += cos(atan2(200-100,200-100)*M_PI/180.0)*1.0;
    			
    				//t1[i].tiros[j].rect.x += sin(atan2(t1[i].rect.y-y,t1[i].rect.x-x)*M_PI/180.0)*1.0;
	            	//t1[i].tiros[j].rect.y += cos(atan2(t1[i].rect.y-y,t1[i].rect.x-x)*M_PI/180.0)*1.0;
					//printf("X: %d, Y: %d\n", x, y);
...
int distancia(SDL_Rect zumbi, SDL_Rect torre)
{
	int xzumbi = zumbi.x;
	int yzumbi = zumbi.y;
