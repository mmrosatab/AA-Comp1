...
int main(void)
{
	...
	while(loop)
	{
		if(SDL_PollEvent(&event))
		{
            ...
            {
            	/*
            	if((event.button.x >= 0 && event.button.x <= 40) && (event.button.y >= 0 && event.button.y <= 40))
            	{
            		ativarSelecao = TRUE;
    	...
    	// tiro
      	...
      	for(int j = 0;j < MAX;j++)
      	{
         	if(torre1.tiros[j].tiroAtivo == 1)
         	{
         		...
         		//torre1.tiros[j].rect.x -= sin(0*M_PI/180.0)*5.0;
            	//torre1.tiros[j].rect.y -= cos(0*M_PI/180.0)*5.0;

            	torre1.tiros[j].rect.x -= sin(0);
            	torre1.tiros[j].rect.y -= cos(0);
