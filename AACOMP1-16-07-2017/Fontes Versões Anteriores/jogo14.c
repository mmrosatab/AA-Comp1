...
void carregarSurfaces()
{
	...
	torre = IMG_Load("Imagens/torre4.png");
...
void inicializarTiros(TORRE torre)
{
	...
	for(int j = 0;j < MAX;j++)
	{
		torre.tiroEsq[j].tiroAtivo = 0;
		torre.tiroDir[j].tiroAtivo = 0;
		torre.tiroCima[j].tiroAtivo = 0;
		torre.tiroBaixo[j].tiroAtivo = 0;
...
int main(void)
{
	...
	for (int i = 0; i < MAX; ++i)
	{
		torre1.tiroEsq[i].surface = IMG_Load("Imagens/tiro1.png");
		torre1.tiroDir[i].surface = IMG_Load("Imagens/tiro1.png");
		torre1.tiroCima[i].surface = IMG_Load("Imagens/tiro1.png");
		torre1.tiroBaixo[i].surface = IMG_Load("Imagens/tiro1.png");
		torre1.tiroEsq[i].angulo = 270;
		torre1.tiroDir[i].angulo = 90;
		torre1.tiroCima[i].angulo = 0;
		torre1.tiroBaixo[i].angulo = 180;
	}
	...
	while(loop)
	{
			...
			if(event.type == SDL_KEYDOWN)
			{
				switch(event.key.keysym.sym)
				{
					...
					case SDLK_SPACE:
						...
						torre1.tiroEsq[i].tiroAtivo = 1;
						torre1.tiroDir[i].tiroAtivo = 1;
						torre1.tiroCima[i].tiroAtivo = 1;
						torre1.tiroBaixo[i].tiroAtivo = 1;
						
						torre1.tiroEsq[i].rect.x = (torre1.rect.x+12);
						torre1.tiroEsq[i].rect.y = (torre1.rect.y+5);

						torre1.tiroDir[i].rect.x = (torre1.rect.x+12);
						torre1.tiroDir[i].rect.y = (torre1.rect.y+5);

						torre1.tiroCima[i].rect.x = (torre1.rect.x+12);
						torre1.tiroCima[i].rect.y = (torre1.rect.y+5);

						torre1.tiroBaixo[i].rect.x = (torre1.rect.x+12);
						torre1.tiroBaixo[i].rect.y = (torre1.rect.y+5);
    	...
    	// tiro
      	...
      	for(int j = 0;j < MAX;j++)
      	{
         	if(torre1.tiroCima[j].tiroAtivo == 1)
         	{
         	
         		torre1.tiroCima[j].rect.x -= sin(torre1.tiroCima[j].angulo*M_PI/180.0)*5.0;
            	torre1.tiroCima[j].rect.y -= cos(torre1.tiroCima[j].angulo*M_PI/180.0)*5.0;
				
            	if(controlarBlitTiro(torre1.tiroCima[j].rect))
					torre1.tiroCima[j].tiroAtivo = 0;
				else
					SDL_BlitSurface(torre1.tiroCima[j].surface, NULL, tela, &torre1.tiroCima[j].rect);

			}
			
			if(torre1.tiroBaixo[j].tiroAtivo == 1)
         	{
         	
         		torre1.tiroBaixo[j].rect.x -= sin(torre1.tiroBaixo[j].angulo*M_PI/180.0)*5.0;
            	torre1.tiroBaixo[j].rect.y -= cos(torre1.tiroBaixo[j].angulo*M_PI/180.0)*5.0;
				
            	if(controlarBlitTiro(torre1.tiroBaixo[j].rect))
					torre1.tiroBaixo[j].tiroAtivo = 0;
				else
					SDL_BlitSurface(torre1.tiroBaixo[j].surface, NULL, tela, &torre1.tiroBaixo[j].rect);

			}

			if(torre1.tiroEsq[j].tiroAtivo == 1)
         	{
         	
         		torre1.tiroEsq[j].rect.x -= sin(torre1.tiroEsq[j].angulo*M_PI/180.0)*5.0;
            	torre1.tiroEsq[j].rect.y -= cos(torre1.tiroEsq[j].angulo*M_PI/180.0)*5.0;
				
            	if(controlarBlitTiro(torre1.tiroEsq[j].rect))
					torre1.tiroEsq[j].tiroAtivo = 0;
				else
					SDL_BlitSurface(torre1.tiroEsq[j].surface, NULL, tela, &torre1.tiroEsq[j].rect);

			}

			if(torre1.tiroDir[j].tiroAtivo == 1)
         	{
         	
         		torre1.tiroDir[j].rect.x -= sin(torre1.tiroDir[j].angulo*M_PI/180.0)*5.0;
            	torre1.tiroDir[j].rect.y -= cos(torre1.tiroDir[j].angulo*M_PI/180.0)*5.0;
				
            	if(controlarBlitTiro(torre1.tiroDir[j].rect))
					torre1.tiroDir[j].tiroAtivo = 0;
				else
					SDL_BlitSurface(torre1.tiroDir[j].surface, NULL, tela, &torre1.tiroDir[j].rect);
      	...
      	if(colocarTorre)
      	{
      		controlarPosicaoTorre(torre1.rect);

			SDL_BlitSurface(torre, NULL, tela, &torre1.rect);	
      	}
