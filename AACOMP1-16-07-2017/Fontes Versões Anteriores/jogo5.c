...
void inicializarPosZumbi()
{
	printf("inicializarPosZumbi\n");
}


// descobrir pq só funciona com ponteiro pq??????
void andarZumbi(SDL_Rect *rectZumbi)
{
	// depois colocar um for para fazer que varios zumbis andem
	if(rectZumbi->x < 160)
	{
...
int jogo()
{
	...
	while(loop)
	{
		...
		andarZumbi(&z1.rect);
      	...
      	SDL_BlitSurface(zumbi, NULL, tela, &z1.rect);	
