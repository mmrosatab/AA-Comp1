...
int andarZumbi()
{
	...
	}else if(rectZumbi.y == 360 && rectZumbi.x != 400)
	{
		rectZumbi.x++;
	...
	while(loop)
	{
		if(SDL_PollEvent(&event))
		{
			...
			}else if(event.type == SDL_MOUSEMOTION)
			{
                ...
                //rectSelecao.y = (int)event.motion.y;
                //rectSelecao.x = (int)event.motion.x;
    	...
    	// tiro
      	...
      	SDL_BlitSurface(zumbi, NULL, tela, &rectZumbi);	
