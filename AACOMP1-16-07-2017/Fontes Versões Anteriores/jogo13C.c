    	...
    	if(t1[i].estado == ATIVADA)
    	{
    		for (int j = 0; j < NUM_TIROS; j++)
    		{
	    		if(t1[i].tiros[j].estado == TRUE)
	        	{
	            	...
	            	t1[i].tiros[j].rect.x -= sin(t1[i].tiros[j].angulo*M_PI/180.0)*1.0;
	            	t1[i].tiros[j].rect.y -= cos(t1[i].tiros[j].angulo*M_PI/180.0)*1.0;
...
void matarZumbi(ZUMBI *zumbi, TIRO *tiros)
{
	for (int i = 0; i < tamOrda; i++)
	{
		for (int j = 0; j < NUM_TIROS; j++)
		{
			if(colisao(zumbi[i].rect,tiros[j].rect))
			{
	...
	for (int i = 0; i < qtdTorres; i++)
	{
		if(t1[i].estado == ATIVADA)
		{
			for (int j = 0; j < NUM_TIROS; j++)
			{
				...
				t1[i].tiros[j].rect.x = x;
				t1[i].tiros[j].rect.y = y;	
				t1[i].tiros[j].estado = TRUE; 
				t1[i].tiros[j].angulo = 0; 
...
void inicializarTiro2(TORRE t,int anguloZumbi)
{
	if(t.estado == ATIVADA)
	{
		for (int j = 0; j < NUM_TIROS; j++)
		{
			...
			t.tiros[j].rect.x = x;
			t.tiros[j].rect.y = y;	
			t.tiros[j].estado = TRUE; 
			t.tiros[j].angulo = anguloZumbi;
		}
		//printf("entrei\n");
	}
}
...
void alcance()
{
	for (int i = 0; i < tamOrda; i++)
	{
		...
		{
			if(distancia(z1[i].rect,t1[j].rect))
			{
				// esta no alcance
				// ativa tiro torre j
				inicializarTiro2(t1[j], 90);
				//seta angulo
