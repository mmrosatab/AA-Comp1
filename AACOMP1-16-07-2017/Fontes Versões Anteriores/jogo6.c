...
void inicializarZumbis(int quant)
{
	for (int i = 0; i < quant; i++)
	{
		z2[i].rect.x = -1 + i*25; // 20 é o espacamento entre os zumbis 
		z2[i].rect.y = 80;
		z2[i].estado = VIVO;
	}
}
...
void andarZumbi2(int quant)
{
	for (int i = 0; i < quant; i++)
	{
		if(z2[i].estado == VIVO)
		{
			...
			{
				zumbi = IMG_Load("Imagens/z1descendo.png");
				z2[i].rect.y++;
			...
			{
				zumbi = IMG_Load("Imagens/z1subindo.png");
				z2[i].rect.y--;
...
void blitarZumbis(int quant)
{
	for (int i = 0; i < quant; ++i)
	{
		if(z2[i].estado == VIVO)
			SDL_BlitSurface(zumbi,&z2[i].sprites[z2[i].frame], tela, &z2[i].rect);
	}
}
...
int jogo()
{
	...
	while(loop)
	{
		...
		blitarMapa();
      	andarZumbi2(tamOrda);
      	controlarFrame(tamOrda);
      	blitarZumbis(tamOrda);
