...
int distancia(SDL_Rect inimigo, SDL_Rect torre)
{
	int xInimigo = inimigo.x;
	int yInimigo = inimigo.y;

	int xTorre = torre.x;
	int yTorre = torre.y;

	double distancia = sqrt(pow(yInimigo-yTorre,2) + pow(xInimigo-xTorre,2));

	if(distancia <= 40) // distancia for <= do que o alcance da torre
	{
		return TRUE;
	}
...
int main(void)
{
	...
	while(loop)
	{
			...
			if(event.type == SDL_KEYDOWN)
			{
				switch(event.key.keysym.sym)
				{
					...
					case SDLK_SPACE:
						...
						torre1.tiros[i].rect.x = (torre1.rect.x+12);
						torre1.tiros[i].rect.y = (torre1.rect.y+5);
		...
		if(distancia(rectSoldado,torre1.rect))
		{
			if (i == MAX) // controle número de tiros
			{
                i = 0;
    	...
    	if(colocarTorre)
      	{
      		int x = torre1.rect.x;
      		int y = torre1.rect.y;
      		int w2 = (torre1.rect.w/2);
      		int h2 = (torre1.rect.h/2);

      		angulo = atan((x+w2) / (y+h2));

      		printf("O angulo eh %f\n",angulo);
      	...
      	for(int j = 0;j < MAX;j++)
      	{
         	if(torre1.tiros[j].tiroAtivo == 1)
         	{
         		...
         		torre1.tiros[j].rect.x -= sin(angulo*M_PI/180.0)*50.0;
            	torre1.tiros[j].rect.y -= cos(angulo*M_PI/180.0)*50.0;
