...
void carregarSurfaces()
{
	...
	bStart = IMG_Load("Imagens/bStart.png");
	bStop = IMG_Load("Imagens/bStop.png");
	...
	*/
	...
	if(!bStart)
	{
		printf("Nao foi possivel carregar imagem bStart\n");
	}
	if(!bStop)
	{
		printf("Nao foi possivel carregar imagem bStop\n");
	}
}
...
void inicializarEstadoZumbis()
{
	...
	{
		z2[i].estado = MORTO;
		z2[i].surface = IMG_Load("Imagens/z1.png");
	}
...
void andarZumbi2()
{
	for (int i = 0; i < tamOrda; i++)
	{
		...
		if(z2[i].estado == VIVO)
		{
			if(z2[i].rect.x < 165)
			{
				z2[i].surface = IMG_Load("Imagens/z1lado.png");

				z2[i].rect.x++;
			}
			else if(z2[i].rect.x == 165 && z2[i].rect.y != 360)
			{
				z2[i].surface = IMG_Load("Imagens/z1descendo.png");
				z2[i].rect.y++;
			...
			{
				z2[i].surface = IMG_Load("Imagens/z1subindo.png");
				z2[i].rect.y--;
...
void blitarZumbis()
{
	...
	{
		if(z2[i].estado == VIVO)
			SDL_BlitSurface(z2[i].surface,&z2[i].sprites[z2[i].frame], tela, &z2[i].rect);
	}
}
	...
	while(loop)
	{
		if(SDL_PollEvent(&event))
		{
			...
			}else if(event.type == SDL_MOUSEMOTION)
            {
            	...
            	if((event.motion.x >= 800 && event.motion.x <= 860) && (event.motion.y >= 23 && event.motion.y <= 67))
            	{
            		printf("mouse em cima botao start\n");
            		
            	}
            	if((event.motion.x >= 800 && event.motion.x <= 860) && (event.motion.y >= 71 && event.motion.y <= 110))
            	{
            		printf("mouse em cima botao stop\n");
            ...
            }else if(event.type == SDL_MOUSEBUTTONDOWN)
            {
            	...
            	if((event.button.x >= 800 && event.button.x <= 860) && (event.button.y >= 23 && event.button.y <= 67))
            	{
            		
                    colocarOrda = 1;
            	}

            	if((event.button.x >= 800 && event.button.x <= 860) && (event.button.y >= 71 && event.button.y <= 110))
            	{
            		colocarOrda = 0;
                    pausaOrda = 1;
            	}
      ...
      // Lê a fila de eventos e põe o evento mais antigo em "event"
        while (SDL_PollEvent(&event)) // Loop de eventos
        {
            ...
            }else if(event.type == SDL_MOUSEMOTION)
            {
            	...
            	if((event.motion.x >= 698 && event.motion.x <= 848) && (event.motion.y >= 63 && event.motion.y <= 117))
            	{
            ...
            }else if(event.type == SDL_MOUSEBUTTONDOWN)
            {
            	...
            	if((event.button.x >= 698 && event.button.x <= 848) && (event.button.y >= 63 && event.button.y <= 117))
            	{
